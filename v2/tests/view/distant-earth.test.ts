...
describe('the ground line', () => {
  ...
  it('is off while the real ground covers it, and on once it starts to leave', () => {
    ...
    /*
      MOVED AT M9.14, and it is the assertion moving rather than a tolerance
      loosening: this line read `expect(distantEarthVisible(100, 200)).toBe(true)`
      and encoded a threshold of 0.05, which is ten metres at the intro's zoom.
      A hundred metres out of a two-hundred-metre frame is EXACTLY where the
      true ground line reaches the bottom edge, so it is the first ratio at
      which a stand-in for the ground is not simply a second one drawn over it.
      Below it the layer is now off entirely — see `distantEarthVisible` for why
      an orthographic side view has no horizon to draw.
    */
    expect(distantEarthVisible(100, 200)).toBe(false);
    expect(distantEarthVisible(101, 200)).toBe(true);
    expect(distantEarthVisible(50_000, 1_000)).toBe(true);
    expect(distantEarthVisible(NaN, 200)).toBe(false);
  });

  it('is off entirely while the real ground is still on screen', () => {
    // The whole of M9.14: on the pad you are looking ALONG the ground, and what
    // is in front of you is buildings and sky. Not a horizon.
    for (const altitude of [0, 1, 10, 42, 84, 99.9]) {
      expect(distantEarthOpacity(altitude, 200), `${altitude} m`).toBe(0);
      expect(distantEarthVisible(altitude, 200), `${altitude} m`).toBe(false);
    }
  });

  it('fades in over the climb rather than arriving in one frame', () => {
    const span = GROUND_LEAVES_SCREEN + DISTANT_EARTH_FADE_SPAN;
    expect(distantEarthOpacity(200 * GROUND_LEAVES_SCREEN, 200)).toBe(0);
    expect(distantEarthOpacity(200 * span, 200)).toBeCloseTo(1, 6);
    expect(distantEarthOpacity(200 * span * 4, 200)).toBeCloseTo(1, 6);
    // Monotonic, and genuinely partway through in the middle rather than
    // snapping at either end.
    let previous = -1;
    for (let i = 0; i <= 20; i++) {
      const here = distantEarthOpacity(200 * (GROUND_LEAVES_SCREEN + (i / 20) * DISTANT_EARTH_FADE_SPAN), 200);
      expect(here).toBeGreaterThanOrEqual(previous);
      previous = here;
    }
    const middle = distantEarthOpacity(200 * (GROUND_LEAVES_SCREEN + DISTANT_EARTH_FADE_SPAN / 2), 200);
    expect(middle).toBeGreaterThan(0.3);
    expect(middle).toBeLessThan(0.7);
    expect(distantEarthOpacity(NaN, 200)).toBe(0);
    expect(distantEarthOpacity(1000, 0)).toBe(0);
  });
});

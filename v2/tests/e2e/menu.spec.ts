...
test('the menu opens, offers every preset, and closes', async ({ page }) => {
  ...
  for (const id of [
    ...
    await expect(page.locator(`[data-preset="${id}"]`), id).toBeVisible();
  }

  await page.locator('[data-menu-control="close"]').click();
...
test('a preset fills the form without flying it', async ({ page }) => {
  ...
  await page.locator('[data-preset="booster-sep"]').click();
  await expect(page.locator('[data-field="altitude"]')).toHaveValue('70000');
  await expect(page.locator('[data-field="speedX"]')).toHaveValue('1130');
  ...
  await expect(page.locator('[data-readout="altitude"] .unit')).toHaveText('M');

  await page.locator('[data-menu-control="clear"]').click();
  await expect(page.locator('[data-field="altitude"]')).toHaveValue('');
});
...
test('Configure starts the new flight and closes the menu', async ({ page }) => {
  ...
  const altitude = page.locator('[data-readout="altitude"]');
...
test('typing in the editor does not fire the engines', async ({ page }) => {
  ...
  const ids = ['rcs', 'fins', 'raptor0', 'raptor1', 'raptor2'];
  const lit = async () =>
    Promise.all(
      ids.map(async (id) =>
        ((await page.locator(`[data-indicator="${id}"]`).getAttribute('class')) ?? '').includes(
          'is-on',
        ),
      ),
    );
  ...
  await page.locator('[data-field="altitude"]').fill('31337');
  ...
  await expect(page.locator('[data-field="altitude"]')).toHaveValue('31337123');
});
...
test('time warp changes how fast the flight runs', async ({ page }) => {
  ...
  const rate = page.locator('[data-menu-control="timeRate"]');
  ...
  await page.locator('[data-menu-control="timeDirection"]').click();
...
test('the random-failure toggle lights and persists into a new flight', async ({ page }) => {
  ...
  const toggle = page.locator('[data-menu-control="randomFailure"]');
  ...
  await expect(page.locator('[data-menu-control="randomFailure"]')).toHaveClass(/is-on/);
});

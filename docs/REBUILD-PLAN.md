...
### The M7 goal prompt (done — Depth and Speed, planned and built 2026-08-25)

```
/goal Depth and Speed is finished: all seven M7 tasks in docs/ROADMAP-TASKS.md are checked off in order, each to its acceptance line, built to the design in docs/DEPTH-AND-SPEED-PLAN.md, one id-prefixed commit per task, all pushed to origin claude/first-project-rebuild-bjniik. Proof shown in command output at the end: (1) cd v2 && npm run lint && npm run test && npm run build all exit 0 and npx playwright test green across ALL FIVE projects, phone viewports included; (2) git diff v2/src/core against the M7 start commit prints nothing and the seven golden digests in tests/golden/unification.test.ts are byte-identical to their M2.14 values — this is a graphics milestone; (3) the trajectory map's projection is replayed over all seven golden fixtures with no NaN and no off-canvas coordinate, and its predicted-touchdown error against the actual touchdown is measured and reported for the goldens that land, with the orbital presets showing no-solution rather than a wrong number; (4) the compression curves — distant earth and velocity streaks — are unit-tested at the altitudes and speeds the seven scenarios actually visit, monotonic and continuous, and each names itself a compression in its own comment per the plan's honesty rule; (5) the frame path is unchanged in kind: one rAF subscriber, binder benchmark under 2 ms WITH the map redraw included, zero-allocation test extended to the map and the streaks, and the particle pool headroom reported against the 576-of-4000 baseline; (6) the budget report shows first-load JS <= 250 kB gzip and fonts <= 80 kB, and fresh screenshots at 1 km, 20 km and 100 km plus desktop and phone are committed with the README updated. Constraints throughout: compression is allowed in the depiction and never in the numbers — every value on the trajectory map is read from SimState or computed by core at true scale; the frozen tree at v2/tests/fixtures/legacy/ is never modified; Svelte renders on interaction only. M7.3 retunes the camera control law under the owner decision of 2026-08-25 (plan § 6.1) and is sequenced third on purpose, because M7.4 and M7.6 are drawn into the frame it defines: the bit-identical guarantee is gone and its five replacement properties are the acceptance line — vehicle stays framed over all seven goldens, damped not springy, frame-rate independent, deterministic, never below the ground — with the altitude FOV at the moderate ~5x setting and the curve flat at 1x below 500 m, so the intro and every landing are untouched by construction. The trajectory map is always-on and collapsible, starting collapsed on a phone. This goal is also met if progress becomes impossible without a new owner decision — in that case the final message states exactly what is blocked, why, and what decision is needed.
```

### The M8 goal prompt (done — Sound, planned and built 2026-08-25)

```
/goal Sound is finished: all five M8 tasks in docs/ROADMAP-TASKS.md are checked off in order, each to its acceptance line, built to the design in docs/SOUND-PLAN.md, one id-prefixed commit per task, all pushed to origin claude/first-project-rebuild-bjniik. Proof shown in command output at the end: (1) cd v2 && npm run lint && npm run test && npm run build all exit 0 and npx playwright test green across all five projects; (2) git diff v2/src/core against the M8 start commit prints nothing and the seven golden digests are byte-identical to their M2.14 values — sound is an output, and if a value is needed that SimState lacks it is derived in audio/, never added to core; (3) the SEVENTH WALL is live and tested: core/ may not import from audio/, proved by a violation fixture through ESLint's Node API exactly as the other six are, with its scoping tested too; (4) the parameter curves are pure functions of SimState pinned at the throttle settings, engine counts, dynamic pressures and altitudes the seven scenarios actually reach, and an OfflineAudioContext render asserts the buffer's RMS is higher at 100% throttle than at 40% and falls toward its floor as the vehicle climbs out of the atmosphere — assertions rather than opinions; (5) the graph is built once and its node count is asserted constant over a long flight, writes are diffed and counted against stubs like every other binder, and nothing allocates on the frame path; (6) the budget report shows first-load JS <= 250 kB gzip, fonts <= 80 kB and audio <= 250 kB with audio decoded lazily so it never blocks the first frame, every sample precached and the full offline playthrough e2e still green. Constraints throughout: the context starts SUSPENDED and unlocks on the first gesture, so the intro plays silently until the player takes control — that is the design, not a compromise; muting suspends the context rather than zeroing a gain; a browser that throws on localStorage must not break the simulator; the frozen tree at v2/tests/fixtures/legacy/ is never modified. State plainly in the final message that no test covers whether it SOUNDS good — that is a listening decision for the owner. This goal is also met if progress becomes impossible without a new owner decision — in that case the final message states exactly what is blocked, why, and what decision is needed.
```

### The M9 goal prompt (queued — Graphics, planned 2026-08-26; four owner decisions recorded)

```
/goal Graphics is finished: all NINE M9 tasks in docs/ROADMAP-TASKS.md are checked off in order, each to its acceptance line, built to the design in docs/GRAPHICS-PLAN.md, one id-prefixed commit per task, all pushed to origin claude/first-project-rebuild-bjniik. Four owner decisions of 2026-08-26 are binding and are recorded in the plan: (a) M9.4 audits every unit annotation in core/ and is the ONE exception to the frozen-core rule, bounded to comments and taken in its own commit; (b) all nine tasks run straight through in one goal; (c) the camera gives up only when crashed; (d) airframe shake lands at M7.3's designed SHAKE_FRACTION = 0.006 rather than dialled back. Proof shown in command output at the end: (1) cd v2 && npm run lint && npm run test && npm run build all exit 0 and npx playwright test green across all five projects, phone viewports included; (2) git diff v2/src/core against the M9 start commit contains COMMENT LINES AND NOTHING ELSE, shown in full, with M9.4's commit the only one touching core at all, and the seven golden digests byte-identical to their M2.14 values — this is a pixels milestone and M9.4 is a documentation commit, not a physics one; (3) THE VEHICLE-IN-FRAME INVARIANT over all seven scenarios at every sampled second, shown RED before M9.2's fix and GREEN after, because that test failing on reentry today is the finding this whole milestone starts from; (4) the view is driven by SIMULATED elapsed time rather than wall frameTime, proved by a dropped-frame test and a time-warp test that bound the camera error in BOTH directions, with M7.3's five camera properties still holding plus a sixth — recovery from an error seeded past the give-up radius — and the give-up branch now reached only when crashed; (5) the Q range test asserts every dynamic-pressure threshold in view/, audio/ and hud/ lies inside the interval the seven goldens actually visit, camera shake at max-Q is reported non-zero for the first time at the designed amplitude, and the fin-vortex ramp no longer saturates at 4% of the structural limit; (6) M9.4 reports every unit comment it checked and every one it corrected, each justified against the expression that produces the value, and says so explicitly where a unit genuinely cannot be pinned down rather than guessing; (7) the pixel harness measures rather than eyeballs — plume extent in vehicle-heights, tone spread across the cloud deck and the ground band, smoke and fire separating in a colour histogram — with NO golden-image diffing anywhere; (8) the budget report shows first-load JS <= 250 kB gzip, fonts <= 80 kB, audio <= 250 kB and the ASSET budget byte-identical, because every texture added is generated at runtime and no new art file ships; (9) peak live particle count reported against M7's 576-of-4000 baseline and the frame path still allocation-free. Constraints throughout: no new art assets, no renderer rewrite, no post-processing rebuild; the pooled particle contract holds and new emitters draw from the same fixed pool; everything generated uses seeded hashes so two players see the same sky; every existing test in tests/view/clouds.test.ts passes UNMODIFIED, and if one has to change, the change is the finding and it gets said out loud; the frozen tree at v2/tests/fixtures/legacy/ is never modified; the pig is at x = 0. State plainly in the final message that no test covers whether it LOOKS good — that is a viewing decision for the owner. This goal is also met if progress becomes impossible without a new owner decision — in that case the final message states exactly what is blocked, why, and what decision is needed.
```

### The M6 goal prompt (done — the Broadcast overhaul, landed 2026-08-25)
...
Task-level breakdown with acceptance lines lives in `docs/ROADMAP-TASKS.md` (77 tasks through M9).
...
- ~~**Sound**~~ — **resolved 2026-08-25: it is M8.** Planned in `docs/SOUND-PLAN.md`,
  built after M7. Synthesis for everything continuous, samples only for transients,
  a seventh wall and a new budget line.
- ~~**Graphics quality**~~ — **raised 2026-08-26 by the owner: it is M9.** Planned in
  `docs/GRAPHICS-PLAN.md`. The investigation found a bug before it found a look problem:
  the `reentry` preset draws the vehicle 1734 px off a 1280 px frame, because the view
  integrates wall time while the vehicle integrates simulated time. Fixes first, a pixel
  harness second, then particles, clouds and ground.

...
### The M9 goal prompt (queued — Graphics, planned 2026-08-26)

```
/goal Graphics is finished: all eight M9 tasks in docs/ROADMAP-TASKS.md are checked off in order, each to its acceptance line, built to the design in docs/GRAPHICS-PLAN.md, one id-prefixed commit per task, all pushed to origin claude/first-project-rebuild-bjniik. Proof shown in command output at the end: (1) cd v2 && npm run lint && npm run test && npm run build all exit 0 and npx playwright test green across all five projects, phone viewports included; (2) git diff v2/src/core against the M9 start commit prints nothing — or, if the owner granted the plan § 6 exception, exactly one comment-only hunk correcting the psi/kPa JSDoc — and the seven golden digests are byte-identical to their M2.14 values, because this is a pixels milestone; (3) THE VEHICLE-IN-FRAME INVARIANT over all seven scenarios at every sampled second, shown RED before M9.2's fix and GREEN after, because that test failing on reentry today is the finding this whole milestone starts from; (4) the view is driven by SIMULATED elapsed time rather than wall frameTime, proved by a dropped-frame test and a time-warp test that bound the camera error in both directions, with M7.3's five camera properties still holding plus recovery from an error seeded past the give-up radius; (5) the Q range test asserts every dynamic-pressure threshold in view/, audio/ and hud/ lies inside the interval the seven goldens actually visit, and camera shake at max-Q is reported non-zero for the first time; (6) the pixel harness measures rather than eyeballs — plume extent in vehicle-heights, tone spread across the cloud deck and the ground band, smoke and fire separating in a colour histogram — with NO golden-image diffing anywhere; (7) the budget report shows first-load JS <= 250 kB gzip, fonts <= 80 kB, audio <= 250 kB and the ASSET budget byte-identical, because every texture added is generated at runtime and no new art file ships; (8) peak live particle count reported against M7's 576-of-4000 baseline and the frame path still allocation-free. Constraints throughout: no new art assets, no renderer rewrite, no post-processing rebuild; the pooled particle contract holds and new emitters draw from the same fixed pool; everything generated uses seeded hashes so two players see the same sky; the frozen tree at v2/tests/fixtures/legacy/ is never modified; the pig is at x = 0. State plainly in the final message that no test covers whether it LOOKS good — that is a viewing decision for the owner. This goal is also met if progress becomes impossible without a new owner decision — in that case the final message states exactly what is blocked, why, and what decision is needed.
```
...
Task-level breakdown with acceptance lines lives in `docs/ROADMAP-TASKS.md` (76 tasks through M9).

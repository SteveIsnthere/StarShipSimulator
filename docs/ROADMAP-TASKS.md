...
- 2026-09-02 · M11.6 · Camera modes. Four modes, one law: `follow` (the cockpit's semi-sticky
  ...
  **The gate, corrected.** The commit `c716e65` says the browser suite was green; it was not — its
  ...
  commit that follows.
- 2026-09-02 · M11.7 · Real stars. The 320 brightest of the Yale Bright Star Catalogue (Hoffleit &
  Warren, 5th revised edition, read as the JSON transcription at
  github.com/aduboisforge/Bright-Star-Catalog-JSON; sorted by magnitude, cut at 3.59, RA/Dec to a
  hundredth of a degree) replace the 220 seeded randoms. `view/stars.ts` places them for StarBase
  by hour angle — sidereal time is the sun's local solar hour plus twelve, the sun's right
  ascension at the September equinox the sun module states — and projects the northern half of
  the dome, which is what a side view with east to the right sees: ±70° of azimuth about north
  across the frame, altitude to 70° up it, a horizon at 0.62 of the height. The stars turn a
  degree every four minutes of flight and the Graphics is redrawn when sidereal time has moved a
  quarter of a degree, never per frame; no parallax, because a star is at infinity. Named stars
  are keyed by HR number, and the asterism tests are closed-form facts: Polaris at the latitude
  at every hour, a star on the observer's declination through the zenith at transit, the
  Plough's pointers five pointer-lengths from Polaris, Orion's belt collinear at a degree and a
  half, the Southern Cross six degrees tall. The data module was generated by a script
  (magnitude sort of the JSON, 320 rows, HR-keyed names checked against known positions before
  it was written), not typed.
  **Two defects, both found by review.** Local sidereal time was the solar hour PLUS twelve,
  where the two twelves cancel — the sun module measures its hour angle as `hour − 12` and the
  sun's right ascension at the September equinox is twelve — so the sky was half a day out and
  every night flight showed the opposite hemisphere: Vega and Deneb below the horizon on a
  September evening, with Regulus, a March star, at 69 degrees. The only test covering the
  mapping asserted the formula and so blessed it; it now asserts the SKY — the Summer Triangle
  high at nine in September, Sirius up in the small hours — which an arithmetic slip cannot
  satisfy. And the projection was anisotropic, azimuth across the width and altitude up the
  height independently, which sheared the Plough by 3.8× between the desktop and phone
  viewports; it is one scale in both axes now, the altitude band fixed and the azimuth field
  following the frame's aspect, so a wide window sees more sky either side of north and a
